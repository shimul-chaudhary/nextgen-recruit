...
def apply_button_clicked(job_title, company_name, location, row_index,description,flag,status):
  ...
  if search_term and search_term.lower() in job_title.lower():  
    if flag==False:
        ...
        if st.button(f"Apply", key=f"apply_button_{row_index}"):
            apply_button_clicked(job_title, company_name, location, row_index,description,flag,status)
